...
async function listUserFeatureMatches(userId){const a=await listFeatureMatches('all');return a.filter(m=>m.players.some(p=>String(p.user_id)===String(userId))).map(m=>({...m,room_id:m.players.length>=2?m.room_id:'',room_password:m.players.length>=2?m.room_password:'',my_slot:m.players.find(p=>String(p.user_id)===String(userId))?.slot}));}
