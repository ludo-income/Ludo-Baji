...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='GET'&&p==='/api/user/matches'){const x=await userAuth(req,res);if(!x)return;return send(res,200,{ok:true,matches:await db.listFeatureMatches(String(u.query.status||'all'))});}
 if(req.method==='GET'&&p==='/api/user/matches/mine'){const x=await userAuth(req,res);if(!x)return;return send(res,200,{ok:true,matches:await db.listUserFeatureMatches(x.id)});}
 if(req.method==='GET'&&p.startsWith('/api/user/matches/')&&p.split('/').length===5){const x=await userAuth(req,res);if(!x)return;const m=await db.getFeatureMatch(decodeURIComponent(p.split('/')[4]));if(!m)return send(res,404,{ok:false,error:'Match not found'});return send(res,200,{ok:true,match:m});}
