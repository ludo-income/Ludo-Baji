...
function otpBlocked(key){const now=Date.now(),x=otpRequestAttempts.get(key);if(!x)return false;if(now-x.first>60*1000){otpRequestAttempts.delete(key);return false}return x.count>=5}
